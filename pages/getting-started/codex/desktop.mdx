...
- 已获取有效 API Key（参考 [获取 API Key](/getting-started/claude-code/get-api-key)）
...
Codex 启动时会从操作系统环境变量读取 `OPENAI_API_KEY`；如果读不到，即使 `auth.json` 里已经有 Key，认证也可能失败。

### 5.1 CLI 终端（必须）

在终端里运行 Codex CLI，或调试连通性，都需要先设好环境变量：

<Tabs items={['macOS / Linux (zsh)', 'macOS / Linux (bash)', 'Windows (PowerShell)', 'Windows (CMD)']}>
  <Tabs.Tab>
    **永久生效（推荐）：**
    ```bash
    echo 'export OPENAI_API_KEY="你的APIKey"' >> ~/.zshrc
    source ~/.zshrc
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    **永久生效（推荐）：**
    ```bash
    echo 'export OPENAI_API_KEY="你的APIKey"' >> ~/.bashrc
    source ~/.bashrc
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    **永久生效（推荐）：**
    ```powershell
    [System.Environment]::SetEnvironmentVariable('OPENAI_API_KEY', '你的APIKey', 'User')
    ```
    *设置后需重启 PowerShell 窗口。*
  </Tabs.Tab>
  <Tabs.Tab>
    **永久生效（推荐）：**
    ```cmd
...
### 5.2 macOS GUI 进程（如需通过 launchctl 注入）

macOS GUI 应用（包括 Codex Desktop）**不会**读取 `~/.zshrc` / `~/.bashrc` 中的 `export`——这是 macOS 终端 shell 与 Aqua GUI 之间的环境隔离造成的。如果 Desktop 需要从环境变量读取 Key，可以使用：
...
<Callout type="info">
  如果你已经通过 `config.toml` + `auth.json` 完成配置并能正常对话，**本小节可跳过**。重启系统后 `launchctl setenv` 会失效，如需持久化请配合 LaunchAgents。
</Callout>
...
<Tabs items={['macOS / Linux', 'Windows (PowerShell)']}>
  <Tabs.Tab>
    ```bash
    curl -sS "https://claude-code.club/openai/responses" \
  ...
  <Tabs.Tab>
    ...
    curl.exe -sS "https://claude-code.club/openai/responses" `
      -H "Authorization: Bearer 你的APIKey" `
      -H "Content-Type: application/json" `
      -d $body
    ```
...
| `stream disconnected before completion: stream closed before response.completed` | `base_url` 配置有误。检查 `config.toml` 中 `base_url` 是否为 `https://claude-code.club/openai`，Codex CLI 会自动拼接 `/responses` 构成最终请求地址 |
...
- [获取 API Key](/getting-started/claude-code/get-api-key) — claude-code.club API Key 申请流程
- [配置 Base URL 和 Auth Token](/getting-started/claude-code/faq/config-baseurl-authtoken) — 环境变量 / 配置文件 / 项目级配置三种方式
