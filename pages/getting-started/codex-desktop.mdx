...
本指南面向已安装 **Codex Desktop**（GUI 客户端）的用户，帮你把桌面端连接到 OpenAI 兼容网关 `https://claude-code.club/openai/v1`。如果你只用 Codex CLI，请优先参考 [安装和配置](/getting-started/codex-installation)。
...
| `stream disconnected before completion: stream closed before response.completed` | `base_url` 末尾少了 `/v1`。Codex CLI（v0.130 起）按 `{base_url}/responses` 拼 URL，少 `/v1` 会请求到 `/openai/responses`，gateway 返回空 SSE 流并立即关闭。改为 `https://claude-code.club/openai/v1` 即可 |
| 仍然访问旧网关 | 检查 `config.toml` 中 `base_url` 是否还是旧的 `https://cc.ai-code.club`，改为 `https://claude-code.club/openai/v1` |
